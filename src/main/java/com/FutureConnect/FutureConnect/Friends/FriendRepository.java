...
public interface FriendRepository extends JpaRepository<Friends, Long> {
  ...
  @Query(
      "SELECT f FROM Friends f WHERE (f.user1.id = :userId OR f.user2.id = :userId) AND f.status = :status")
  List<Friends> findByUserIdAndStatus(
      @Param("userId") UUID userId, @Param("status") FriendshipStatus status);

  @Query(
      "SELECT CASE WHEN COUNT(f) > 0 THEN true ELSE false END FROM Friends f WHERE "
